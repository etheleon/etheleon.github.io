...
If you take a look inside the tools directory of the community edition (MEGAN CE) installation, you’ll find a bash executable called `blast2lca` [MEGAN6 Community Edition github repository](https://github.com/danielhuson/megan-ce/blob/master/tools/blast2lca) which taps into the java classes. Below’s the printout from the command line tool.

It's been kindly included into the Community Edition of MEGAN (MEGAN CE) and its extremely valuable as a tool for one to basically access the core algorithms within MEGAN:
1. Lowest Common Ancestor algorithm and 
2. Functional (KEGG/COG/eggNOG) functional assignment.

MEGAN's been around for awhile with its 1st release way back in (2007) and newest iteration MEGAN6 it includes quite a few additions to deal with increasingly large datasets. However, I would say it’s still meant for desktop users and if you need to run any huge jobs on multiple large sequencing projects, you'll be hardpressed.
...
1. Combine Annotations - How does one combined KO and TAXONOMIC annotations such that we will have a combined annotation for each query (short read/long read/contig)
2. gi2ko map file generator - KEGG annotations. In the community edition, the tool to generate the mapping file (gi kegg) is not included like in the ultimate edition. What if you’re not in a position to pay for the ultimate edition license (which includes with it the KEGG database) or have a older version of KEGG lying around somewhere in the server, what should you do?
3. fullPipeline - How to go all the way from the tabbed blast output to the combined table mentioned above
...
You’ll be able to specify the locations of the mapping files (KEGG and taxonomy as well as the path to the executable)
...
Although MEGAN UE provides a KEGG mapper generating tool, it doesnt take into consideration how NCBI has assigned a unique GI to each representative sequence in the non-redundant database (NCBI NR) under which are “duplicate” sequence GIs and ref IDs
...
We’ve separately included in the tools folder of the python package the [ref2kegg.go nr-gi to kegg ortholog KO mapping file generator](https://github.com/etheleon/MEGAN/blob/master/tools/ref2kegg.go) and you could pass the mapping file via the `--gi2kegg` flag. At the time of writing the parser is written in golang (typed lang) to increase the speed of parsing the NR fa file.
